  ...
  <section className="py-20 px-6" style={{ background: 'transparent' }}>
    <div className="max-w-md mx-auto space-y-8">
      <div className="text-center space-y-2">
        <p className="font-serif-en text-xs tracking-[0.4em] uppercase" style={{ color: '#9BB0C8' }}>Gift Registry</p>
        <h2 className="font-khmer-title text-xl" style={{ color: '#2C3E56', lineHeight: 1.7 }}>អំណោយអាពាហ៍ពិពាហ៍</h2>
        ...
        <p className="font-khmer-body text-sm" style={{ color: '#7A8EA8' }}>
          ប្រសិនបើអ្នកចង់ផ្ញើអំណោយ អ្នកអាចធ্វើប្រតិបត្តិការតាមរយៈ :
        </p>
      </div>
      ...
      {registries.map((item, i) => (
        ...
        >
          <div className="px-5 py-3 flex items-center justify-between"
            style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(106, 140, 178, 0.3)' }}>
            <span className="font-serif-en text-xs font-bold tracking-widest uppercase" style={{ color: '#E8C97A' }}>{item.bankName}</span>
          ...
          <div className="p-5 flex gap-4 items-center">
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="font-khmer-body font-bold" style={{ color: '#FAF6EF' }}>{item.accountName}</p>
              </div>
              <div>
                <p className="font-serif-en text-[10px] tracking-widest uppercase" style={{ color: 'rgba(232, 201, 122, 0.7)' }}>Account Number</p>
                <div className="flex items-center gap-3">
                  <p className="font-serif-en font-bold tracking-widest" style={{ color: '#FAF6EF', fontSize: '1rem' }}>{item.accountNumber}</p>
                  <button
                    onClick={() => copyToClipboard(item.accountNumber, item.id)}
                    className="p-2 rounded-lg transition-colors flex items-center gap-1.5"
                    style={{ background: copiedId === item.id ? '#E8C97A' : 'rgba(232, 201, 122, 0.15)', color: copiedId === item.id ? '#141c27' : '#E8C97A' }}
                  >
