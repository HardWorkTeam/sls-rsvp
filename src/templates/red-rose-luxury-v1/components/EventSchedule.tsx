    ...
    >
      <div className="max-w-2xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-3">
          ...
          {lunarDateText ? (
            <p className="font-khmer-body font-semibold text-xs md:text-sm text-[#E8C97A]/90">
              {lunarDateText}
            </p>
          ) : null}
        ...
        {dayGroups.map((group, gi) => (
          <div key={group.date} className="space-y-4">
            ...
            >
              {multiDay ? (
                <p className="font-khmer-body font-bold text-sm tracking-wider text-[#E8C97A]">
                  ថ្ងៃទី {toKhmerNumber(gi + 1)} · Day {gi + 1}
                </p>
              ) : null}
              {formatBeSolar(group.date) ? (
                <p className="font-khmer-body text-xs md:text-sm text-[#E8C97A]/90">
                  {formatBeSolar(group.date)}
                </p>
              ) : null}
            </m.div>
