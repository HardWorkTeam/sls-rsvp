...
export const EventSchedule: React.FC<EventScheduleProps> = ({ events, lunarDateText }) => {
  ...
  const themeColor = '#D4A020'; // Amber gold matching Angkor
  const textColor = '#FAF6EF'; // Light text inside container
    ...
    >
      <div className="max-w-2xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-3">
          <h2 className="font-khmer-title text-2xl md:text-3xl text-white leading-relaxed">
            កម្មវិធីមង្គលអាពាហ៍ពិពាហ៍
          </h2>
          {lunarDateText ? (
            <p className="font-khmer-body font-semibold text-xs md:text-sm text-white/90">
              {lunarDateText}
            </p>
          ) : null}
          <div className="h-[1px] w-28 mx-auto" style={{ background: 'linear-gradient(90deg, transparent, #ffffff, transparent)' }} />
        </div>
          ...
          <div key={group.date} className="space-y-4">
            ...
            >
              ...
              {formatBeSolar(group.date) ? (
                <p className="font-khmer-body text-xs md:text-sm text-white/90">
                  {formatBeSolar(group.date)}
                </p>
              ) : null}
            </m.div>
                ...
                <div className="relative space-y-0.5 max-w-xl mx-auto">
                  {group.events.map((evt, index) => (
                    ...
                    >
                      ...
                      {/* 3. Text Column */}
                      <div className="py-4 space-y-1 text-left text-white">
                        <span className="font-khmer-body font-bold text-xs tracking-wider text-white">
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-[10px] font-khmer-body font-bold tracking-wider uppercase hover:opacity-80 mt-1 cursor-pointer text-white underline"
                        >
